...
const getSkillBackground = (name: string) => {
    ...
    if (lower.includes('java')) return '/src/assets/java.png';
    if (lower.includes('frontend') || lower.includes('react')) return '/src/assets/react.jpeg';
    if (lower.includes('python')) return '/src/assets/python.webp';
    if (lower.includes('desktop') || lower.includes('c++')) return '/src/assets/cpp.jpg';
    if (lower.includes('web3') || lower.includes('solidity')) return '/src/assets/solidity.png';
    if (lower.includes('mobile') || lower.includes('native')) return 'https://images.unsplash.com/photo-1512941937669-90a1b58e7e9c?auto=format&fit=crop&q=80&w=1000';
    if (lower.includes('database')) return 'https://images.unsplash.com/photo-1544383835-2ce296c09b27?auto=format&fit=crop&q=80&w=1000';
    if (lower.includes('devops') || lower.includes('tooling')) return 'https://images.unsplash.com/photo-1667372393119-3d4c48d07fc9?auto=format&fit=crop&q=80&w=1000';
    return undefined;
};
                    ...
                    <div className="flex items-center justify-between pt-6 border-t border-white/5">
                        <div className="flex -space-x-3">
                            {skill.icons?.slice(1).map((icon, i) => (
                                <div key={i} className="w-10 h-10 rounded-full border-2 border-[#0B1120] bg-[#161F31] p-2 backdrop-blur-sm">
